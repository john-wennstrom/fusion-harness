...
describe("model stack", () => {
  ...
  test("object-form config parses global runtime and resolves relative extension paths", () => {
    const body = `
extensions:
  - npm:semantic-tools
...
`;
  ...
  test("per-slot override fields parse independently", () => {
    const body = `
extensions:
  - npm:semantic-tools
tools:
...
`;
  ...
  test("per-slot tools support inherit/include/exclude rule objects", () => {
    const body = `
tools:
  read:
    - semantic_find
...
slots:
  ...
  - name: main
    ...
    primary: true
`],
  ])("rejects object-form runtime config error: %s", (_label, body) => {
    expect(() => loadModelStack(fixture(body).file)).toThrow("model-stack config invalid");
  });
});
